...
inline void CheckInit(IDXGISwapChain* pSwapChain) {
    ...
    if (!bInitialized &&
        ...
        MainWindow()->set_renderer(MainRenderer());
        MainRenderer()->Init(pSwapChain, pDevice, pContext);
        bInitialized = true;
    }
}
...
bool FunctionTable::Initialize() {
    ...
    if (p_window->get_hwnd() == nullptr) return false;
